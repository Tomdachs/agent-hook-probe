...
The Claude adapter exposes only the built-in `Read` tool, explicitly denies MCP tools, uses `--restricted`, `--permission-mode dontAsk`, and disables permission prompts. The canary already exists and is never modified. Bash, Edit, and Write are not exposed to the probe turn, and `--dangerously-skip-permissions` is never used. Managed Claude policy remains authoritative.
